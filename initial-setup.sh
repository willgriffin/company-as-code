...
# Function to generate AWS SES SMTP password from secret key
# Uses pure shell implementation with standard Unix tools
generate_ses_smtp_password() {
    local secret_key="$1"
    local message="SendRawEmail"
    
    # Create the key by prepending version byte (0x04) to the secret key
    local key_with_version=$(printf '\x04%s' "$secret_key")
    
    # Use OpenSSL to generate HMAC-SHA256 and encode as base64
    if command -v openssl >/dev/null 2>&1; then
        printf '%s' "$message" | openssl dgst -sha256 -hmac "$key_with_version" -binary | base64
    else
        echo "ERROR: openssl not available for SMTP password generation" >&2
        return 1
    fi
}

# Function to create SES DNS records in Terraform configuration
create_terraform_ses_dns_records() {
    local domain="$1"
    local ses_region="$2"
    
    echo "    Updating Terraform AWS configuration to include domain: $domain"
    
    # Update terraform/aws/terraform.tfvars with the domain
    local tfvars_file="terraform/aws/terraform.tfvars"
    
    # Create the tfvars file if it doesn't exist
    if [[ ! -f "$tfvars_file" ]]; then
        mkdir -p "$(dirname "$tfvars_file")"
        cat > "$tfvars_file" << EOF
# Auto-generated by initial-setup.sh
domain = "$domain"
project_name = "$SETUP_REPO_PROJECT_NAME"
aws_region = "$ses_region"
ses_verified_domains = ["$domain"]
EOF
    else
        ...
        if ! grep -q "^project_name" "$tfvars_file"; then
            echo "project_name = \"$SETUP_REPO_PROJECT_NAME\"" >> "$tfvars_file"
        fi
        if ! grep -q "^aws_region" "$tfvars_file"; then
            echo "aws_region = \"$ses_region\"" >> "$tfvars_file"
        fi
        if ! grep -q "^ses_verified_domains" "$tfvars_file"; then
            echo "ses_verified_domains = [\"$domain\"]" >> "$tfvars_file"
        fi
    fi
    
    echo -e "${GREEN}✓ Terraform AWS configuration updated${NC}"
}
...
replace_placeholders() {
    ...
    if [[ -d "flux/clusters/my-cluster" ]]; then
        echo "  Renaming cluster directory: my-cluster -> $SETUP_REPO_CLUSTER_NAME"
    ...
    find . -type f \( -name "*.yaml" -o -name "*.yml" -o -name "*.tf" -o -name "*.md" -o -name "*.sh" \) \
        -not -path "./.git/*" \
        -not -name "initial-setup.sh" \
        -exec grep -l -E "({{SETUP_REPO_|clusters/my-cluster|example\.com)" {} \; | while read -r file; do
        ...
        sed \
            ...
            -e "s|example\.com|$SETUP_REPO_DOMAIN|g" \
            "$file" > "$temp_file"
...
setup_infrastructure() {
    ...
    cleanup_on_failure() {
        ...
        if [[ $exit_code -ne 0 ]]; then
            ...
            # Check if CREATED_RESOURCES is defined and not empty
            ...
            for resource in "${reversed_resources[@]}"; do
                ...
                case "$resource_type" in
                    "spaces-bucket")
                        ...
                        # Extract region from bucket name or use default
                        local delete_region="${SETUP_REPO_SPACES_REGION:-nyc3}"
                        # Use AWS CLI with Spaces credentials if available
                        if [[ -n "${SPACES_ACCESS_KEY:-}" ]] && [[ -n "${SPACES_SECRET_KEY:-}" ]]; then
                            ...
                            AWS_ACCESS_KEY_ID="$SPACES_ACCESS_KEY" AWS_SECRET_ACCESS_KEY="$SPACES_SECRET_KEY" \
                                ...
                                --endpoint-url "https://${delete_region}.digitaloceanspaces.com" >/dev/null 2>&1 || true
                            
                            # Then delete the bucket
                            if AWS_ACCESS_KEY_ID="$SPACES_ACCESS_KEY" AWS_SECRET_ACCESS_KEY="$SPACES_SECRET_KEY" \
                                aws s3api delete-bucket --bucket "$resource_id" \
                                --endpoint-url "https://${delete_region}.digitaloceanspaces.com" >/dev/null 2>&1; then
                                echo -e "    ${GREEN}✓ Deleted${NC}"
                            else
                                echo -e "    ${RED}✗ Failed to delete${NC}"
                            fi
                        else
                            echo -e "    ${YELLOW}⚠ No Spaces credentials available for deletion${NC}"
                        fi
                        ;;
                    "spaces-key")
                        echo "  Deleting Spaces access key: $resource_id"
                        if doctl spaces keys delete "$resource_id" >/dev/null 2>&1; then
                            echo -e "    ${GREEN}✓ Deleted${NC}"
                        else
                            echo -e "    ${RED}✗ Failed to delete${NC}"
                        fi
    ...
    # Set trap for cleanup on error only (not exit to avoid double execution)
    ...
    local required_tools=("doctl" "aws" "gh" "jq")
    local missing_tools=()
    ...
    if ! doctl auth init --access-token "$DIGITALOCEAN_TOKEN" >/dev/null 2>&1; then
        echo -e "${RED}✗ Invalid DigitalOcean token${NC}"
        echo "Please check that your DIGITALOCEAN_TOKEN is correct"
        echo "You can generate a new token at: https://digitalocean.pxf.io/je2Ggv"
        return 1
    fi
    ...
    # Generate deterministic bucket name
    # Format: <cluster-name>-<repo-name>-tf
    local repo_name=$(basename $(git remote get-url origin) .git)
    local bucket_name="${SETUP_REPO_CLUSTER_NAME}-${repo_name}-tf"
    local spaces_region="${SETUP_REPO_SPACES_REGION:-nyc3}"
    
    # Check if bucket exists and handle deletion if needed
    local bucket_exists=false
    
    # We'll need to use AWS CLI with Spaces credentials later
    # For now, let's create the bucket using the Spaces access keys we'll generate
    
    # First, let's generate the Spaces access keys using doctl
    echo "  Creating Spaces access keys..."
    local spaces_key_response
    spaces_key_response=$(doctl spaces keys create "${bucket_name}-key" --grants "bucket=;permission=fullaccess" --output json 2>&1)
    
    if [[ $? -eq 0 ]]; then
        local spaces_access_key spaces_secret_key spaces_key_id
        
        # Extract key information from doctl output (it returns an array)
        spaces_access_key=$(echo "$spaces_key_response" | jq -r '.[0].access_key')
        spaces_secret_key=$(echo "$spaces_key_response" | jq -r '.[0].secret_key')
        spaces_key_id=$(echo "$spaces_key_response" | jq -r '.[0].access_key')  # Use access_key as the ID
        
        # Validate we got actual values
        if [[ "$spaces_access_key" == "null" ]] || [[ -z "$spaces_access_key" ]]; then
            echo -e "${RED}✗ Failed to extract access key from response${NC}"
        ...
        echo "  Access Key ID: ${spaces_access_key:0:10}..."  # Show first 10 chars for debugging
        
        # Export for later use
        export SPACES_ACCESS_KEY="$spaces_access_key"
        export SPACES_SECRET_KEY="$spaces_secret_key"
        
        # Give the keys a moment to propagate
        echo "  Waiting for keys to propagate..."
        sleep 2
    else
        echo -e "${RED}✗ Failed to create Spaces access keys${NC}"
        echo "Error: $spaces_key_response"
        return 1
    fi
    
    # Now create the bucket using AWS CLI with Spaces endpoint
    echo "  Creating Spaces bucket '$bucket_name'..."
    
    # Check if bucket exists
    if AWS_ACCESS_KEY_ID="$spaces_access_key" AWS_SECRET_ACCESS_KEY="$spaces_secret_key" \
        aws s3api head-bucket --bucket "$bucket_name" \
        --endpoint-url "https://${spaces_region}.digitaloceanspaces.com" 2>/dev/null; then
        
        echo -e "${YELLOW}⚠ Bucket '$bucket_name' already exists${NC}"
        
        # Check if we should delete existing bucket
        local delete_bucket=false
        if [[ "${SETUP_DELETE_EXISTING:-false}" == "true" ]]; then
            delete_bucket=true
        elif [[ "${NON_INTERACTIVE:-false}" != "true" ]]; then
            echo -n "Delete existing bucket and recreate? [y/N]: "
            read -r response
            if [[ "$response" =~ ^[Yy]$ ]]; then
                delete_bucket=true
            fi
        ...
        if [[ "$delete_bucket" == "true" ]]; then
            echo "  Deleting existing bucket contents..."
            AWS_ACCESS_KEY_ID="$spaces_access_key" AWS_SECRET_ACCESS_KEY="$spaces_secret_key" \
                aws s3 rm "s3://$bucket_name" --recursive \
                --endpoint-url "https://${spaces_region}.digitaloceanspaces.com" >/dev/null 2>&1
            
            echo "  Deleting existing bucket..."
            if AWS_ACCESS_KEY_ID="$spaces_access_key" AWS_SECRET_ACCESS_KEY="$spaces_secret_key" \
                aws s3api delete-bucket --bucket "$bucket_name" \
                --endpoint-url "https://${spaces_region}.digitaloceanspaces.com" 2>/dev/null; then
                echo -e "${GREEN}✓ Existing bucket deleted${NC}"
            else
                echo -e "${YELLOW}⚠ Could not delete existing bucket, continuing...${NC}"
            fi
        else
    ...
    if [[ "$bucket_exists" != "true" ]]; then
        local create_output
        if create_output=$(AWS_ACCESS_KEY_ID="$spaces_access_key" AWS_SECRET_ACCESS_KEY="$spaces_secret_key" \
    ...
    # We already have the Spaces access keys from earlier
    # Just use the exported variables SPACES_ACCESS_KEY and SPACES_SECRET_KEY
    ...
    local repo_name=$(basename $(git remote get-url origin) .git)
    local smtp_user="${SETUP_REPO_CLUSTER_NAME}-${repo_name}-ses-smtp"
    ...
    local policy_name="${SETUP_REPO_CLUSTER_NAME}-${repo_name}-ses-policy"
    local policy_doc='{
        "Version": "2012-10-17",
        "Statement": [
            {
    ...
    # Delete any existing access keys for this user (AWS allows max 2 keys per user)
    local existing_keys
    existing_keys=$(aws iam list-access-keys --user-name "$smtp_user" --query 'AccessKeyMetadata[].AccessKeyId' --output text 2>/dev/null)
    if [[ -n "$existing_keys" ]]; then
        echo "    Cleaning up existing access keys..."
        for key_id in $existing_keys; do
            aws iam delete-access-key --user-name "$smtp_user" --access-key-id "$key_id" >/dev/null 2>&1
            echo "    Deleted existing key: $key_id"
        done
    fi
    ...
    if [[ $? -eq 0 ]]; then
        ...
        # Generate SES SMTP password using pure shell implementation
        # AWS SES SMTP password algorithm: HMAC-SHA256 with version byte (0x04) prepended to secret key
        smtp_password=$(generate_ses_smtp_password "$smtp_secret_key")
    ...
    # Create SES DNS records in Terraform configuration
    echo "  Creating SES DNS records in Terraform configuration..."
    create_terraform_ses_dns_records "$SETUP_REPO_DOMAIN" "$SETUP_REPO_SES_REGION"
    
    # Display DNS records that need to be configured
    echo
    echo -e "${YELLOW}=== DNS Configuration ===${NC}"
    echo "SES DNS records have been automatically added to Terraform configuration."
    echo "These will be created when you deploy your infrastructure:"
    echo
    ...
    echo -e "${BLUE}These DNS records will be automatically created during Terraform deployment.${NC}"
    echo
...
# Function to post completion comment to setup issue
post_setup_completion_comment() {
    echo -e "${BLUE}Posting completion details to setup issue...${NC}"
    
    # Find the setup issue
    local setup_issue
    setup_issue=$(gh issue list --search "configure repository from template" --json number --jq '.[0].number' 2>/dev/null || echo "")
    
    if [[ -z "$setup_issue" ]]; then
        echo -e "${YELLOW}⚠ Could not find setup issue to comment on${NC}"
        return 0
    fi
    
    # Prepare DNS records info
    local dns_records="**DNS Configuration Required:**
The following DNS records need to be added to complete email verification:

1. **SES Domain Verification (TXT record):**
   - Name: \`_amazonses.$SETUP_REPO_DOMAIN\`"
...
2. **DKIM Records (CNAME records):**"
        for token in $dkim_tokens; do
            dns_records="$dns_records
   - \`${token}._domainkey.$SETUP_REPO_DOMAIN\` → \`${token}.dkim.amazonses.com\`"
        done
    fi
    
    # Post the comment
    local comment_body="✅ **Setup completed successfully!**

The initial repository setup has been completed with the following resources created:
- DigitalOcean Spaces bucket: \`$SPACES_BUCKET_NAME\`
- Spaces access keys generated
- AWS SES domain verification initiated for \`$SETUP_REPO_DOMAIN\`
- IAM user \`${SETUP_REPO_CLUSTER_NAME}-ses-smtp\` created with SMTP credentials
- All GitHub repository secrets configured
- Template placeholders replaced throughout the repository
- GitHub production environment created

$dns_records

Once these DNS records are added, domain verification will complete automatically.

**Next Steps:**
1. Add the DNS records listed above
2. Run \`terraform init\` in the \`terraform/\` directory
3. Deploy your infrastructure with \`terraform apply\`
4. Use \`./initial-setup.sh --eject\` to remove template artifacts when ready"
    
    if gh issue comment "$setup_issue" --body "$comment_body" 2>/dev/null; then
        echo -e "${GREEN}✓ Posted completion details to issue #$setup_issue${NC}"
    else
        echo -e "${YELLOW}⚠ Could not post comment to issue #$setup_issue${NC}"
    fi
}
...
cleanup_previous_run() {
    ...
    for resource in "${reversed_resources[@]}"; do
        ...
        case "$resource_type" in
            "spaces-bucket")
                echo "  Deleting Spaces bucket: $resource_id"
                echo -e "    ${YELLOW}⚠ Manual deletion may be required via DigitalOcean console${NC}"
                ;;
            "spaces-key")
                echo "  Deleting Spaces access key: $resource_id"
                if doctl spaces keys delete "$resource_id" >/dev/null 2>&1; then
...
interactive_setup() {
    ...
    setup_infrastructure || { echo "setup_infrastructure failed"; return 1; }
    
    # Process the configuration
    echo "Running replace_placeholders..."
    replace_placeholders || { echo "replace_placeholders failed"; return 1; }
    echo "Running create_cleanup_issue..."
    create_cleanup_issue || { echo "create_cleanup_issue failed"; return 1; }
    ...
    # Post completion details to setup issue
    post_setup_completion_comment
}
...
case "${1:-}" in
    ...
    --help|-h)
        ...
        echo "  SETUP_DELETE_EXISTING=true    Delete existing state/resources (non-interactive)"
        echo
